...
import UppyDashboard from "../upload/UppyDashboard";
import Uppy from "@uppy/core";
import ZipTargetFiles from "../compress/ZipTargetFiles";
...
const UploadStoresModal = ({
    ...
    const { id } = useParams();
    // const createReqData = useSelector((state) => state.createReq.name);

    // useEffect(() => {
    //     console.log('fetchProjectReqList received:', {
    //         type: typeof fetchProjectReqList,
    //         isFunction: typeof fetchProjectReqList === 'function'
    //     });
    // }, [fetchProjectReqList]);
    ...
    const getUploadedFilecount = async (bucket, projectid, requestid) => {
        try {
            ...
            return { count: 0 }; // 👈 never throw here (important)
        }
    };
    ...
    const compressedUpload = async (pid, rid) => {
        try {
            ...
            const key = `public/${pid}/${rid}/file.zip`;
    ...
    const closeModal = () => {
        ...
        if (isSafeToCancel) {
            updateRequest(id, createReqData?.requestid, "cancel", 0, "PSU", files[0]?.name, "", []);
        }
        fetchProjectReqList("projectstores")
        // dispatch(clearCreateReq());
        setOpen(false);
        setStep(1);
        setFiles([]);
        setRawFiles([]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const keyFileName = `public/${id}/${createReqData?.requestid}/${files[0]?.name}`
            updateRequest(id, createReqData?.requestid, 'processing', count, 'PSU', keyFileName, '', []);
            setStep(3);
            setProcess(true);
            setIsPolling(true);

            // if (!updateRes || updateRes.error) {
            //     console.error('updateRequest failed:', updateRes);
    ...
    const handleOk = () => {
        setIsPolling(false);
        // if (typeof fetchProjectReqList === 'function') {
        //     console.log('Calling fetchProjectReqList');
        //     fetchProjectReqList();
        // } else {
        //     console.error('fetchProjectReqList is not a function');
        // }
    ...
    const handleCancel = () => {
        ...
        updateRequest(id, createReqData?.requestid, "cancel", 0, "PSU", "", "", []);
...
export default UploadStoresModal;