...
### StoreUpdate — used when editing a store (`PATCH /stores/{id}`)
...
|---|---|---|
| `404 Not Found` | Store does not exist | `GET`, `PATCH`, or `DELETE` with a store number that is not in the database |
...
6. **Edit an existing store**
   Show a pre-filled form. On save, send only the changed fields to `PATCH /stores/{store_id}`.
...
| Update a store | `PATCH` | `/stores/{store_id}` | JSON with fields to change |
