...
export default function ProjectStoresPage() {
  ...
  const fetchProjectStoreHierarchy = async (filter) => {
    ...
    try {
      ...
      const data = await lambdaGet(`/projectstorehierarchy/${retailerId}/${id}/${filter}`);
      if (!data || data.error) { console.log(data.error); return; }
  ...
  const createProjRequest = async () => {
    try {
      const payload = { filetype: "PSU", projectid: id };
  ...
  const updateRequest = async (projId = '', reqId = '', requestStatus = 'processing', filecount = 0, uploadType = 'CSV', filename = '', requestflow = '', basecallpoints = []) => {
    try {
      ...
      const payload = { projectid: projId || id, requestid: reqId || createReqData?.requestid, requeststatus: requestStatus, filecount, filetype: uploadType, filename, requestflow, basecallpoints, compareid: '', isCompare: '', comparedProject: '' };
  ...
  useEffect(() => {
    if (id && retailerId) fetchProjectStoreHierarchy("region");
  }, [id, retailerId]);
